    ...
    if multithreaded:
        ...
        if TypeCheck.templateUtil.is_number(var_type_dict):
            if TypeCheck.templateUtil.is_external(var_type_dict):
                ...
                variable_type_str = 'waldo.LockedVariables.SingleThreadedLockedNumberVariable'
        elif TypeCheck.templateUtil.is_true_false(var_type_dict):
            if TypeCheck.templateUtil.is_external(var_type_dict):
                variable_type_str = library_transform('WaldoExtTrueFalseVariable')
            else:
                variable_type_str = 'waldo.LockedVariables.SingleThreadedLockedTrueFalseVariable'
        elif TypeCheck.templateUtil.is_text(var_type_dict):
            if TypeCheck.templateUtil.is_external(var_type_dict):
                variable_type_str = library_transform('WaldoExtTextVariable')
            else:
                variable_type_str = 'waldo.LockedVariables.SingleThreadedLockedTextVariable'
...
def get_javaized_map_key_type(var_type_dict):
    '''
    @param {dict} var_type_dict --- Gotten from an AstNode's .type
    field.

    In java implementation, need to know key type to parameterize generic 
    map template type
    '''
    #### DEBUG
    if not TypeCheck.templateUtil.isMapType(var_type_dict):
        emit_assert(
            'Expecting map type in get_map_key_type')
    #### END DEBUG
        
    index_type_dict = TypeCheck.templateUtil.getMapIndexType(var_type_dict)
...
def get_javaized_map_val_type(var_type_dict,multithreaded):
    ...
    In java implementation, need to know val type to parameterize generic 
    map template type
    '''
...
def get_javaized_dewaldoified_map_type(var_type_dict):
    ...
    map template type
    '''
    index_type = get_javaized_map_key_type(var_type_dict)
    
    return 'HashMap<%s,%s>' % (
        index_type,
        dewaldoified_java_type(TypeCheck.templateUtil.getMapValueType(var_type_dict)))

def dewaldoified_java_type(var_type_dict):
    if TypeCheck.templateUtil.is_text(var_type_dict):
        return 'String'
    elif TypeCheck.templateUtil.is_number(var_type_dict):
        return 'Number'
    elif TypeCheck.templateUtil.is_true_false(var_type_dict):
        return 'Boolean'
    elif TypeCheck.templateUtil.isMapType(var_type_dict):
        return get_javaized_dewaldoified_map_type(var_type_dict)
    else:
        emit_assert('Unknown type to dewaldoify')
