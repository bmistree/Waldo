...
public class ReceiveFirstPhaseCommitMessage extends ServiceAction 
{
	...
	public ReceiveFirstPhaseCommitMessage (
			waldo.Endpoint _local_endpoint,String _event_uuid,String _msg_originator_endpoint_uuid,
	...
	public void run() 
	{
		waldo.LockedActiveEvent act_event = local_endpoint._act_event_map.get_event(event_uuid);
		...
		if (act_event != null)
		{
			if (successful)
			{
				act_event.receive_successful_first_phase_commit_msg(
						event_uuid, msg_originator_endpoint_uuid, children_event_endpoint_uuids);
			}
			else
			{
