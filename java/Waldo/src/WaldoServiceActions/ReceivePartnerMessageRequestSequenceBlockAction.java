		...
		ServiceAction 
{
	...
	public ReceivePartnerMessageRequestSequenceBlockAction(
			waldo.Endpoint _local_endpoint,PartnerRequestSequenceBlock _partner_request_block_msg)
	{
	...
	public void run() 
	{
		...
		try
		{
			...
			waldo.LockedActiveEvent evt = local_endpoint._act_event_map.get_or_create_partner_event(uuid,priority);
			evt.recv_partner_sequence_call_msg(partner_request_block_msg);
			
			Util.logger_warn("May want to catch exceptions in Waldo to hide stack trace");
		}
		catch (WaldoExceptions.StoppedException stopped_excep)
		{
