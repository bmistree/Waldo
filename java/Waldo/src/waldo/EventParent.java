...
public abstract class EventParent {
	...
	private java.util.concurrent.locks.ReentrantLock _priority_mutex = new java.util.concurrent.locks.ReentrantLock();
            ...
            boolean partner_contacted, LockedActiveEvent _event)
	{
        ...
        for (String waiting_on_uuid : same_host_endpoints_contacted_dict.keySet())
        {
            ...
            Endpoint endpoint = same_host_endpoints_contacted_dict.get(waiting_on_uuid).endpoint_object;
            ...
            boolean partner_contacted)
	{
		...
		for (EventSubscribedTo endpoint_to_second_phase_commit : same_host_endpoints_contacted_dict.values())
		{
            ...
            boolean partner_contacted, boolean stop_request)
	{
		...
		for (EventSubscribedTo subscribed_elements_to_rollback : same_host_endpoints_contacted_dict.values())
		{
			Endpoint endpoint_to_rollback = subscribed_elements_to_rollback.endpoint_object;
			
			if (endpoint_to_rollback._uuid != backout_requester_endpoint_uuid)
			{
				endpoint_to_rollback._receive_request_backout(uuid,local_endpoint);
			}
		}
