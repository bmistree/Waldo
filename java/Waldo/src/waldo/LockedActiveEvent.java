...
public class LockedActiveEvent {
	...
	private java.util.concurrent.locks.ReentrantLock mutex = new java.util.concurrent.locks.ReentrantLock();
	
	State state = State.STATE_RUNNING;
	
	private java.util.concurrent.locks.ReentrantLock _nfmutex = new java.util.concurrent.locks.ReentrantLock();
    ...
    */
    private java.util.concurrent.locks.ReentrantLock _touched_objs_mutex = 
	 ...
	 * 
	 */
    private java.util.concurrent.locks.ReentrantLock _others_contacted_mutex = 
    		new java.util.concurrent.locks.ReentrantLock();
    ...
    */
    ...
    private java.util.concurrent.ArrayBlockingQueue<SignalFunction>signal_queue = null;
	...
	public LockedActiveEvent(EventParent _event_parent, ActiveEventMap _event_map)
	{
		...
		for (EventSubscribedTo subscribed_to_element : other_endpoints_contacted.values())
		{
			for (java.util.concurrent.ArrayBlockingQueue<EndpointResultObject> res_queue : subscribed_to_element.result_queues)
			{
				...
				if (stop_request)
					...
					queue_feeder = new WaldoCallResults.BackoutBeforeEndpointCallResult();
	 ...
	 * @param stop_request
	 */
	public void backout(String backout_requester_endpoint_uuid, boolean stop_request)
	{
        _lock();
	 ...
	 */
	public void obj_request_backout_and_release_lock(MultiThreadedLockedObject obj_requesting)
	{
            ...
            java.util.concurrent.ArrayBlockingQueue<MessageResultObject>threadsafe_unblock_queue, boolean first_msg)
	{
        ...
        if (state == State.STATE_RUNNING)
        {
            ...
            if (threadsafe_unblock_queue != null)
            {
				...
				//# a response.
                message_listening_queues_map.put(reply_with_uuid, threadsafe_unblock_queue);
            }         
	...
	{
		forward_backout_request_and_backout_self(skip_partner,already_backed_out,false);
	}
