...
public class ExecutingEventContext {
    ...
    */
	...
	public ExecutingEventContext (VariableStore _global_store, VariableStore _sequence_local_store)
	{
    ...
    */
	...
	public Object set_val (Object to_write_to, Object to_write, LockedActiveEvent active_event) throws BackoutException
	{
	 ...
	 */
	public Object get_val_if_waldo(Object val, LockedActiveEvent active_event) throws BackoutException
	{
			...
			String host_uuid, boolean new_peered, boolean new_multi_threaded) throws BackoutException
	{
		...
		if (new_multi_threaded)
		{
			if (String.class.isInstance(val))
				return new LockedVariables.LockedTextVariable(host_uuid,new_peered,(String)val);
		...
		else
		{
			if (String.class.isInstance(val))
				...
				return new LockedVariables.SingleThreadedLockedMapVariable(host_uuid,new_peered,(HashMap)val,false);
			else
			{
				Util.logger_assert("Still must add constructors for lists, endpionts, func objects, etc.");
				//elif isinstance(val,list):
	 ...
	 * @throws BackoutException 
	 */
	public Object de_waldoify(Object val,LockedActiveEvent active_event) throws BackoutException
	{
		if (LockedObject.class.isInstance(val))
	...
	{
		Util.logger_assert("Not handling tuple return types in java branch for now.");
		return null;
	}
	 ...
	 * @throws BackoutException 
        ...
        False otherwise.
	 */
    public boolean assign(Object lhs, Object rhs, LockedActiveEvent active_event) throws BackoutException
    {
    ...
    public boolean assign_on_key(Object lhs,Object key,Object rhs, LockedActiveEvent active_event) throws BackoutException
    {
        ...
        if (LockedVarUtils.is_non_ext_text_var(lhs))
        {
        	...
        	String to_overwrite_string = 
        			...
        			(String)raw_rhs + internal_string.substring(((Number) raw_key).intValue()+1);
        	locked_lhs.set_val(active_event, to_overwrite_string);        	
        }
        else if (LockedVarUtils.is_external_text_variable(lhs))
        {
        	Util.logger_assert("Still have not enabled externals in java branch yet.");
        }
        else if (LockedVarUtils.is_reference_container(lhs))
        {
            ...
            ContainerInterface ref_cont = (ContainerInterface) locked_lhs.get_val(active_event);
    ...
    public Object get_val_on_key(Object to_get_from, Object key, LockedActiveEvent active_event) throws BackoutException
    {
        ...
        ContainerInterface cont = (ContainerInterface) locked_to_get_from.get_val(active_event);
        return cont.get_val_on_key(active_event,raw_key);
    }
     ...
     * @return
     */
    public Iterator get_for_iter(Object to_iter_over, LockedActiveEvent active_event)
    {
    ...
    public String to_text(Object what_to_call_to_text_on, LockedActiveEvent active_event)
    {
    ...
    public void signal_call(LockedActiveEvent active_event, SignalFunction func ,Object...args)
    {
    	Util.logger_assert("Have not converted signal call code in java branch yet.");
    }
    ...
    public int handle_len(Object what_calling_len_on, LockedActiveEvent active_event) throws BackoutException
    {
    	...
    	LockedObject locked_what_calling_len_on = (LockedObject) what_calling_len_on;
    	
    	if (LockedVarUtils.is_non_ext_text_var(what_calling_len_on))
    	{
    		String internal_string = (String)locked_what_calling_len_on.get_val(active_event);
            return internal_string.length();
    	}


    	ContainerInterface cont = (ContainerInterface) (locked_what_calling_len_on.get_val(active_event));
    ...
    /**
        ...
        message.
     */    
    public void hide_sequence_completed_call(Endpoint endpoint, LockedActiveEvent active_event)
    {    
	...
	*/
    public void hide_partner_call(
            ...
            String func_name, boolean first_msg) throws NetworkException, ApplicationException, BackoutException
    {
    	
    	ArrayBlockingQueue<WaldoCallResults.MessageCallResultObject> threadsafe_unblock_queue = 
    			new ArrayBlockingQueue<WaldoCallResults.MessageCallResultObject> (Util.SMALL_QUEUE_CAPACITIES);

    	if (! active_event.issue_partner_sequence_block_call(this, func_name, threadsafe_unblock_queue, first_msg))
    	{
    	...
    	WaldoCallResults.MessageCallResultObject queue_elem = null;
		try {
    	...
    	if (WaldoCallResults.BackoutBeforeReceiveMessageResult.class.isInstance(queue_elem))
    		throw new WaldoExceptions.BackoutException();
    	...
    	else if (WaldoCallResults.ApplicationExceptionCallResult.class.isInstance(queue_elem))
    		throw new WaldoExceptions.ApplicationException("appliaction exception");
    			
    	
    	//means that it must be a sequence message call result
    	WaldoCallResults.SequenceMessageCallResult casted_queue_elem = (WaldoCallResults.SequenceMessageCallResult) queue_elem;
        ...
        if (to_exec_next != null)
        	ExecutingEvent.static_run(to_exec_next, active_event, this, null);        	
        else
        {
    		...
    		Object...args) throws BackoutException, NetworkException, ApplicationException
	{
        ...
        else if (! WaldoCallResults.EndpointCompleteCallResult.class.isInstance(queue_elem))
        {
        	throw new WaldoExceptions.BackoutException();
        }
     ...
     */
    public boolean handle_in_check(Object lhs, Object rhs, LockedActiveEvent active_event)
    {
