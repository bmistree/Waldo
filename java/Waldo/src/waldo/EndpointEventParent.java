...
public class EndpointEventParent extends EventParent {
	...
	ArrayBlockingQueue<WaldoCallResults.EndpointCallResultObject> result_queue = null;
	
	public EndpointEventParent(String _uuid,waldo.Endpoint _parent_endpoint,waldo.Endpoint _local_endpoint,
			ArrayBlockingQueue<WaldoCallResults.EndpointCallResultObject>_result_queue, String _priority)
	{
            ...
            boolean partner_contacted, boolean stop_request)
	{
	    ...
	    copy_other_endpoints_contacted.put(parent_endpoint._uuid, 
	...
	*/
			...
			HashMap<String, ArrayBlockingQueue<MessageCallResultObject>> message_listening_queues_map) 
	{
		if (WaldoExceptions.NetworkException.class.isInstance(error))
			result_queue.add( new WaldoCallResults.NetworkFailureEndpointCallResult(error.toString()));
		else if (WaldoExceptions.ApplicationException.class.isInstance(error))
			result_queue.add(new WaldoCallResults.ApplicationExceptionEndpointCallResult(error.toString()));
		else
			result_queue.add(new WaldoCallResults.ApplicationExceptionEndpointCallResult(error.toString()));
	...
	*/
	@Override
    public void receive_successful_first_phase_commit_msg(String event_uuid,
