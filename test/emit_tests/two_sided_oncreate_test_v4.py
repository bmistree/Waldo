...
def SideA (_waldo_classes,_host_uuid,_conn_obj,*args):
    class _SideA (_waldo_classes["Endpoint"]):
        def __init__(self,_waldo_classes,_host_uuid,_conn_obj,new_num):
            ...
            while True:  # FIXME: currently using infinite retry 
                ...
                _to_return = self._onCreate(_root_event,_ctx ,new_num,[])
        ...
        def _onCreate(self,_active_event,_context,new_num,_returning_to_public_ext_array=None):
        ...
        def read_b_text(self):
            ...
            while True:  # FIXME: currently using infinite retry 
                ...
                # to Waldo variables, and de-waldo-ifying them outside of the
                # transaction might return over-written/inconsistent values.
                _to_return = self._endpoint_func_call_prefix__waldo__read_b_text(_root_event,_ctx ,[])
                # try committing root event
                _root_event.request_commit()
                _commit_resp = _root_event.event_complete_queue.get()
                if isinstance(_commit_resp,self._waldo_classes["CompleteRootCallResult"]):
        ...
        def _endpoint_func_call_prefix__waldo__read_b_text(self,_active_event,_context,_returning_to_public_ext_array=None):
            if _context.check_and_set_from_endpoint_call_false():
            ...
            if _returning_to_public_ext_array != None:
                # must de-waldo-ify objects before passing back
                return _context.flatten_into_single_return_tuple((self._partner_endpoint_msg_func_call_prefix__waldo__start_get_b_text(_active_event,_context,) if _context.set_msg_send_initialized_bit_false() else None) if 0 in _returning_to_public_ext_array else _context.de_waldoify((self._partner_endpoint_msg_func_call_prefix__waldo__start_get_b_text(_active_event,_context,) if _context.set_msg_send_initialized_bit_false() else None),_active_event))


            # otherwise, use regular return mechanism... do not de-waldo-ify
            return _context.flatten_into_single_return_tuple((self._partner_endpoint_msg_func_call_prefix__waldo__start_get_b_text(_active_event,_context,) if _context.set_msg_send_initialized_bit_false() else None))
        ...
        def _partner_endpoint_msg_func_call_prefix__waldo__start_get_b_text(self,_active_event,_context,_returning_to_public_ext_array=None):
            ...
            if not _context.set_msg_send_initialized_bit_true():
                ...
                b_text = self._waldo_classes["WaldoTextVariable"](  # the type of waldo variable to create
                    '9__b_text', # variable's name
                ...
                _context.sequence_local_store.add_var(
                    "9__b_text",_context.convert_for_seq_local(b_text,_active_event,self._host_uuid))
            ...
            _active_event.issue_partner_sequence_block_call(
                _context,'receive_get_b_text',_threadsafe_queue, '_first_msg')
            ...
            return _context.sequence_local_store.get_var_if_exists("9__b_text")
...
def SideB (_waldo_classes,_host_uuid,_conn_obj,*args):
    class _SideB (_waldo_classes["Endpoint"]):
        def __init__(self,_waldo_classes,_host_uuid,_conn_obj,new_end_text):
            ...
            self._global_var_store.add_var(
                '5__end_text',self._waldo_classes["WaldoTextVariable"](  # the type of waldo variable to create
                '5__end_text', # variable's name
                _host_uuid, # host uuid var name
                False,  # if peered, True, otherwise, False
                _context.get_val_if_waldo('jefo' ,_active_event)
            ))
            ...
            while True:  # FIXME: currently using infinite retry 
                ...
                # to Waldo variables, and de-waldo-ifying them outside of the
                # transaction might return over-written/inconsistent values.
                _to_return = self._onCreate(_root_event,_ctx ,new_end_text,[])
                # try committing root event
                _root_event.request_commit()
                _commit_resp = _root_event.event_complete_queue.get()
                if isinstance(_commit_resp,self._waldo_classes["CompleteRootCallResult"]):
        ...
        def _onCreate(self,_active_event,_context,new_end_text,_returning_to_public_ext_array=None):
            if _context.check_and_set_from_endpoint_call_false():
                new_end_text = _context.turn_into_waldo_var_if_was_var(new_end_text,True,_active_event,self._host_uuid,False)
            ...
            _tmp0 = new_end_text
            if not _context.assign(_context.global_store.get_var_if_exists("5__end_text"),_tmp0,_active_event):
                pass
        ...
        def _partner_endpoint_msg_func_call_prefix__waldo__receive_get_b_text(self,_active_event,_context,_returning_to_public_ext_array=None):

            _tmp0 = _context.global_store.get_var_if_exists("5__end_text")
            if not _context.assign(_context.sequence_local_store.get_var_if_exists("9__b_text"),_tmp0,_active_event):
                pass
